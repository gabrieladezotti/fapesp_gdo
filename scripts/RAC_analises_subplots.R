...
#análises:
tratamento <- itirapina_rac_subplots %>%
  filter(tratamento != "MOD")
tratamento <- as_tibble(tratamento)
tratamento$tratamento <- droplevels(tratamento$tratamento)
...
summary(aov(lm(richness_change ~ tratamento*invasora, 
       data = tratamento)))
